...
def random_oil_spills(n_spills, config_file, random_seed=None):
    ...
    for spill in range(n_spills):
        ...
        spill_params["vessel_len"].append(vessel_len)
        spill_params["vessel_mmsi"].append(vessel_mmsi)
        spill_params["vessel_origin"].append(vessel_origin)
        spill_params["vessel_dest"].append(vessel_dest)
        ...
        if vessel_type in ['barge','atb']:
            logging.debug(
            f"{vessel_type}: fuel_capacity={fuel_capacity}, cargo_capacity={cargo_capacity}"
            )
        ...
        spill_params["fuel_capacity"].append(fuel_capacity)
        spill_params["cargo_capacity"].append(cargo_capacity)
...
def get_oil_capacity(oil_attrs, vessel_length, vessel_type, random_generator):
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # We estimate barge and atb fuel and cargo capacities with a 
    # probability weighting developed from AIS information and known i
    # volume capacities of ATBs.  This is different then the length-> 
    # capacity method for estimating the capacity of other vessel types, 
    # so I first estimate atb and barges and then calculate the other 
    # vessel types below.
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    if vessel_type == "atb":
        ...
        logging.debug(f"Barge (fuel, cargo) capacities: {fuel_capacity},{cargo_capacity}")
    else:
